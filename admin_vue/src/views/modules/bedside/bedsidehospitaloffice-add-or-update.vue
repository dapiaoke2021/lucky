    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/bedside/bedsidehospitaloffice/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.hospitalId = data.bedsidehospitaloffice.hospitalId
                this.dataForm.name = data.bedsidehospitaloffice.name
                this.dataForm.createdAt = data.bedsidehospitaloffice.createdAt
                this.dataForm.updatedAt = data.bedsidehospitaloffice.updatedAt
                this.dataForm.deletedAt = data.bedsidehospitaloffice.deletedAt
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/bedside/bedsidehospitaloffice/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
