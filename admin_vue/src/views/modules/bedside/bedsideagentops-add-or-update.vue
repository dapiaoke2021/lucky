    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/bedside/bedsideagentops/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.agentId = data.bedsideagentops.agentId
                this.dataForm.opsId = data.bedsideagentops.opsId
                this.dataForm.createdAt = data.bedsideagentops.createdAt
                this.dataForm.updatedAt = data.bedsideagentops.updatedAt
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/bedside/bedsideagentops/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
